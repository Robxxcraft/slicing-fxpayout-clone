  ...
  return (
    <div className="mb-4 p-6 flex flex-col lg:flex-row bg-white border border-[#DDDDDD] rounded-2xl">
      <div className="shrink-0 pr-0 lg:pr-6 pb-4 lg:pb-0">
        <p className="text-xl 2xl:text-[26px] font-bold text-black/40 uppercase tracking-[5%]">
          Tier {" "}
          {authUser.role === "user" && "Trader"}
          {authUser.role === "affiliator" && "Affiliator"}
        </p>
        <div className="mt-6 flex gap-4">
          <div className="p-3 w-20 2xl:w-[100px] h-fit bg-[#F5F5F5] border border-[#DDDDDD] rounded-2xl">
            <img src="/diamond_31419852.png" alt="diamond-icon" 
              className="object-contain aspect-square" />
          </div>
          <div>
            <div className="px-4 2xl:px-5 py-3 lg:py-1.5 2xl:py-3 w-fit bg-linear-to-t from-dark-primary to-primary rounded-full">
              <p className="uppercase text-[14px] 2xl:text-base font-medium text-white tracking-wider">
                {selectedTemplate[authUser.tier as UserTier].title}
              </p>
            </div>
            <p className="mt-2 text-2xl 2xl:text-[28px] font-bold">
