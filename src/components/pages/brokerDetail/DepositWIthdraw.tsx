...
const DepositWIthdraw = () => {
  return (
    <section className="mt-10 md:mt-12 2xl:mt-16 py-8 md:py-10 2xl:py-14 px-5 xl:px-24 2xl:px-56">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 2xl:gap-14">

        {/* DEPOSIT & WITHDRAW */}
        <div id="deposit-withdraw" className="scroll-mt-18 lg:scroll-mt-10 w-full">
          ...
          <div className="mt-5 md:mt-8 2xl:mt-10 border border-[#A9A9A9] overflow-hidden rounded-2xl">
            <table className="table-auto w-full text-[#1D2433] text-base 2xl:text-xl">
              <thead>
                <tr>
                  <th className="px-4 md:px-6 2xl:px-8 py-4 2xl:py-6 bg-[#F1F3F9] font-semibold tracking-[2%] text-left">
                    Metode
                  </th>
                  <th className="px-4 md:px-6 2xl:px-8 py-4 2xl:py-6 bg-[#F1F3F9] font-semibold tracking-[2%] text-left">
                    Waktu
                ...
                {paymentMethods.map((method, idx) => (
                  <tr key={idx}>
                    <td className={`${idx % 2 === 0 ? "bg-white" : "bg-[#F8F9FC]"} 
                      px-4 md:px-6 2xl:px-8 py-3 2xl:py-4 text-left`}
                    >
                      {method.method}
                    </td>
                    <td className={`${idx % 2 === 0 ? "bg-white" : "bg-[#F8F9FC]"} 
        ...
        {/* PLATFORM */}
        <div id="platform" className="scroll-mt-18 lg:scroll-mt-10 w-full">
          ...
          <div className="mt-5 md:mt-8 2xl:mt-10 flex flex-col gap-2.5 2xl:gap-3.5">
            {platforms.map((platform, idx) => (
              <div key={idx} className="flex gap-3 2xl:gap-4 items-center">
                <img src={`/brokerDetail/${platform.icon}`} alt={`Profil ${platform.username}`} 
                  className="size-14 2xl:size-16 rounded-[10px]" />
                <div className="bg-linear-to-t from-dark-primary to-primary border border-transparent bg-clip-border rounded-lg overflow-hidden">
                  <div className="px-4 md:px-5 2xl:px-6 py-2 2xl:py-3 bg-my-light-blue">
                    <p className="text-sm 2xl:text-base font-semibold text-transparent bg-linear-to-t from-dark-primary to-primary bg-clip-text">
                        {platform.username}
                    </p>
                  </div>
                </div>
