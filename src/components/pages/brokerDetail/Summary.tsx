...
const Summary = ({ 
  ...
  return (
    <section id="summary" className="scroll-mt-18 lg:scroll-mt-0 pt-6 md:pt-10 px-6 md:px-11 lg:px-18 xl:px-24 3xl:px-56">
      ...
      <div className="mt-6 3xl:mt-8 grid grid-cols-1 lg:grid-cols-2 gap-3 lg:gap-4 3xl:gap-6">
        {summaryItems.map((item, idx) => (
          <div key={idx} className="px-5 3xl:px-8 py-4 3xl:py-6 flex gap-6 bg-[#F4F6FA] rounded-lg shadow-[0_3px_13.1px_0_rgba(0,0,0,0.1)] border-s-10 border-primary">
            <div className="w-fit">
              <BoundedIcon icon={`/brokerDetail/${item.icon}`} alt="Icon" />
            </div>
            <div className="flex w-fit flex-col gap-2 3xl:gap-4">
              <p className="text-base 3xl:text-xl leading-6 uppercase">{t(item.title)}</p>
              <p className="text-xl 3xl:text-2xl font-semibold leading-6">
